...
export default function ClientsActions({ client }: { client: any }) {
      ...
      {modalType && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
          <div className="bg-white w-full max-w-sm rounded-[2.5rem] p-8 shadow-2xl animate-in zoom-in-95">
            ...
            <h3 className="text-2xl font-black text-slate-900 capitalize mb-2">{modalType} Client?</h3>
            <p className="text-slate-500 text-sm mb-8 leading-relaxed">
              Are you sure you want to {modalType} <strong>@{client.username}</strong>? 
            ...
            <div className="flex flex-col gap-3">
              <button 
                onClick={async () => {
                   if(modalType === 'delete') await deletePro(client.id);
                   else await updateProStatus(client.id, modalType);
                   closeModals();
                }}
